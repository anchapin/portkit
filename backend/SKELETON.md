...
*deps: db.base, fastapi, httpx, pydantic, services.cache, sqlalchemy*
...
### `backend/src/api/model_conversion.py`
> MMSD Model Conversion API Endpoints
*deps: api._authz, converters.mmsd_converter, db.base, db.models, fastapi, fastapi.security, pydantic, security.auth*
...
`async def model_convert(request: ModelConvertRequest, current_user: Optional[User] = Depends(get_current_user_optional), _: bool = Depends(re...)`
  *Model-based conversion using trained GRPO models.*

`async def model_status(current_user: Optional[User] = Depends(get_current_user_optional), _: bool = Depends(require_feature_flag('mmsd_model...)`
  *Check Ollama and model status.*

`async def model_metrics(current_user: Optional[User] = Depends(get_current_user_optional), _: bool = Depends(require_feature_flag('mmsd_model...)`
...
### `backend/src/services/batch/models.py`
> Batch Queue Models
...
### `backend/src/services/batch/queue_manager.py`
> Batch Queue Manager
...
### `backend/src/services/breach_notification.py`
> Data Breach Notification Service for portkit.
*deps: __future__, pydantic*
...
**class BreachNotificationService:**
  *Detects breaches, logs them, and triggers notifications.*
  `def __init__(self, sender: NotificationSender | None = None, security_contact: str = DEFAULT_SECURITY_CONTACT, privacy_contact: st...) -> None`
  ...
  `async def detect_breach(self, severity: BreachSeverity, scope: str, affected_users: int, affected_data_types: list[str], description: str, so...) -> BreachLogEntry`
    *Create, log, and alert on a newly detected breach.*
  `async def send_breach_notification(self, event: BreachEvent, recipient: str, user_id: str | None = None, is_to_authority: bool = False, actions_taken: s...) -> BreachNotification`
