...
FORBIDDEN_API_PATTERNS = [
    ...
    (
        ...
        "createLightningBolt — non-existent method, use dimension.spawnEntity",
    ),
    (r"\.spawnLightning\(", "spawnLightning — non-existent method"),
    (
        r"\.registerEvent\(",
        "registerEvent — wrong API, use .subscribe() on afterEvents/beforeEvents",
    ),
    ...
    (
        r"event\.level\.",
        "event.level — non-existent property chain, use event.source or event.dimension",
    ),
    ...
    (
        ...
        "Server.getInstance — Java singleton pattern, does not exist in Bedrock",
    ),
    (
        r"\.getTileEntity\(\).*?\.getInventory\(",
        "getTileEntity().getInventory — chained method that doesn't exist",
    ),
    ...
    (
        r"\bPlayerEvent\b(?!\.)",
        "PlayerEvent — ambiguous class, use PlayerAfterEvents or PlayerBeforeEvents",
    ),
    (
        r"from\s+['\"]minecraft/server['\"]",
        "Wrong import path — must be '@minecraft/server' (with @)",
    ),
...
class MMSDHallucinationValidator:
    ...
    def validate(self, bedrock_script: str) -> ValidationResult:
            ...
            for line_num, line in enumerate(lines, 1):
                ...
                for match in line_matches:
                    ...
                    if any(
                        ...
                        severity = (
                            "semantic" if "@minecraft/server" not in matched_text else "lingering"
                        )
                    else:
                        severity = "hard"
        ...
        hallucination_rate = len(hallucinated_apis) / max(
            1, len(valid_apis) + len(hallucinated_apis)
        )
...
if __name__ == "__main__":
    ...
    print(
        result.regeneration_signal
        if result.regeneration_signal
        else result.get_hallucination_summary(result)
    )
