...
class CurriculumConfig:
    ...
    def __post_init__(self):
        if self.phase1_weights is None:
            self.phase1_weights = {Difficulty.EASY: 1.0, Difficulty.MEDIUM: 0.0, Difficulty.HARD: 0.0}
        if self.phase2_weights is None:
            self.phase2_weights = {Difficulty.EASY: 0.5, Difficulty.MEDIUM: 0.5, Difficulty.HARD: 0.0}
        if self.phase3_weights is None:
            # Issue #1616: Weight harder examples higher as training progresses
            self.phase3_weights = {Difficulty.EASY: 0.2, Difficulty.MEDIUM: 0.3, Difficulty.HARD: 0.5}
...
def _interpolate_weights(
    weights1: dict, weights2: dict, progress: float
) -> dict:
