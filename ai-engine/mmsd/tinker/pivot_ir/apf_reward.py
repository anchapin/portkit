...
from pivot_ir.schema import PivotIR
...
def compute_apf_reward(
    ...
    # ─────────────────────────────────────────────────────────────────
    has_any_translation = (
        entity_score > 0 or event_score > 0 or api_score > 0 or structure_score > 0.5
    )
...
def estimate_entity_coverage(completion: str) -> float:
    ...
    # Check for manifest
    has_manifest = bool(
        re.search(r'"format_version"', completion) and re.search(r'"header"', completion)
    )

    # Check for entity definitions
    has_entities = bool(
        re.search(r'"minecraft:\w+"', completion) or re.search(r"Entity\s*\(", completion)
    )
