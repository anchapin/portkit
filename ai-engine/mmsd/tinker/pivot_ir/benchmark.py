...
from pivot_ir.schema import PivotIR, pivot_ir_to_dict
from pivot_ir.java_parser import JavaToPivotIRAdapter
from pivot_ir.bedrock_emitter import PivotIRToBedrockAdapter
...
def print_benchmark_report(results: list[BenchmarkResult]) -> None:
    ...
    if len(by_method) >= 2:
        ...
        bleu_diff = sum(r.bleu_score for r in results_b) / len(results_b) - sum(
            r.bleu_score for r in results_a
        ) / len(results_a)
        time_diff = sum(r.generation_time_ms for r in results_b) / len(results_b) - sum(
            r.generation_time_ms for r in results_a
        ) / len(results_a)
...
def compare_direct_vs_pivot(test_cases: list[dict]) -> dict:
    ...
    comparison = {
        ...
        "direct": {
            ...
            "avg_entity_coverage": sum(r.entity_coverage for r in direct_results)
            / len(direct_results),
            "avg_event_coverage": sum(r.event_coverage for r in direct_results)
            / len(direct_results),
        ...
        "pivot_ir": {
            ...
            "avg_entity_coverage": sum(r.entity_coverage for r in pivot_results)
            / len(pivot_results),
