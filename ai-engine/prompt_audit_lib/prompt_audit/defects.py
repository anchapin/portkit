...
class DefectTaxonomy:
    ...
    def update_status(self, defect_id: str, new_status: DefectStatus) -> bool:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "by_severity": {
                self.SEVERITY_LABELS[sev]: len(self.get_by_severity(sev)) for sev in DefectSeverity
            },
            "by_type": {dt.value: len(self.get_by_type(dt)) for dt in DefectType},
            "by_status": {st.value: len(self.get_by_status(st)) for st in DefectStatus},
    ...
    def get_fix_summary(self) -> List[Dict[str, Any]]:
        ...
        return [{"file": fp, "issues": issues} for fp, issues in fix_summary.items()]

    def convert_from_audit(self, audit_findings: List[Any], round_num: int = 1) -> None:
            ...
            if hasattr(finding, "check_name"):
                ...
                if defect_type:
                    severity = severity_map.get(
                        str(finding.severity).lower(), DefectSeverity.MEDIUM
                    )
                    self.add_defect(
